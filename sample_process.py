...
def save_files():
    with h5py.File('datasets/trajectories_normalized_cond.h5', 'r') as h5f:
        rx0 = np.array(h5f.get('min'))
        rx1 = np.array(h5f.get('max'))
    # Generate filenames that don't exist
    pos_base_path = '/home/rotogni/diffusion-lagr/pos_samples_256x40x3_cond'
    init_base_path = '/home/rotogni/diffusion-lagr/init_samples_256x10x3_cond'

    pos_filename = get_available_filename(pos_base_path, '.npy')
    init_filename = get_available_filename(init_base_path, '.npy')

    pos = (np.load('/home/rotogni/diffusion-lagr/samples_256x40x3.npz')['arr_0']+1)*(rx1-rx0)/2 + rx0
    # save positions as a file
    np.save(pos_filename, pos)
    # save initial conditions for next sample as a file
    init = pos[:,29:39,:] - pos[:,30:40,:] 
